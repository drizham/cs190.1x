...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    parsedLine = np.fromstring(line, sep=',')
    return LabeledPoint(parsedLine[0], parsedLine[1:])

# parsedSamplePoints = rawData.map(parsePoint) # this works too!
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda x: getLabeledPrediction(w, x)) #getLabeledPrediction(weights, observation)
